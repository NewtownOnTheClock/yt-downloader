...
def check_info():
    ...
    if url:
        title_print = subprocess.run(['yt-dlp', '--print', "%(title)s", url], check=True, capture_output=True, text=True, shell=True)
        title_clean = title_print.stdout.split("\n")
        title_compliled = [i for i in title_clean]
        result = subprocess.run(['yt-dlp', '-F', url], check=True, capture_output=True, text=True, shell=True)
